...
const (
	...
	ContentBlockTypeServerSchemaToolCall    ContentBlockType = "server_schema_tool_call"
	ContentBlockTypeServerSchemaToolResult  ContentBlockType = "server_schema_tool_result"
...
type ContentBlock struct {
	...
	// FunctionToolCall contains the invocation details for a client-defined, client-executed tool.
	// The client defines the tool schema and executes the tool locally.
	FunctionToolCall *FunctionToolCall `json:"function_tool_call,omitempty"`

	// FunctionToolResult contains the result returned from a client-defined, client-executed tool call.
	FunctionToolResult *FunctionToolResult `json:"function_tool_result,omitempty"`

	// ServerSchemaToolCall contains the invocation details for a server-defined, client-executed tool.
	// The model provider defines and publishes the tool schema (e.g. bash, text_editor, computer),
	// and the model is pre-trained on these schemas for better reliability.
	// However, actual execution is still performed by the client.
	ServerSchemaToolCall *ServerSchemaToolCall `json:"server_schema_tool_call,omitempty"`

	// ServerSchemaToolResult contains the result returned from a server-defined, client-executed tool call.
	ServerSchemaToolResult *ServerSchemaToolResult `json:"server_schema_tool_result,omitempty"`

	// ServerToolCall contains the invocation details for a server-defined, server-executed tool.
	// The model provider defines and executes the tool entirely on the server side
	// (e.g. web_search, code_execution). The client only enables the tool; execution
	// and result injection happen before the response reaches the client.
	ServerToolCall *ServerToolCall `json:"server_tool_call,omitempty"`

	// ServerToolResult contains the result returned from a server-defined, server-executed tool.
...
type ServerSchemaToolCall struct {
	// Name specifies the server-side schema-based tool invoked.
	Name string `json:"name"`

	// CallID is the unique identifier for the tool call.
	CallID string `json:"call_id,omitempty"`

	// Arguments are the raw inputs to the server-side schema-based tool,
	// supplied by the component implementer.
	Arguments any `json:"arguments,omitempty"`
}

type ServerSchemaToolResult struct {
	// Name specifies the server-side schema-based tool invoked.
	Name string `json:"name"`
	...
	// Result refers to the raw output generated by the server-side schema-based tool,
	// supplied by the component implementer.
...
type serverToolCallVariant interface {
	ServerToolCall | ServerToolResult | ServerSchemaToolCall | ServerSchemaToolResult
}
...
func NewContentBlock[T contentBlockVariant](content *T) *ContentBlock {
	...
	case *ServerSchemaToolCall:
		return &ContentBlock{Type: ContentBlockTypeServerSchemaToolCall, ServerSchemaToolCall: b}
	case *ServerSchemaToolResult:
		return &ContentBlock{Type: ContentBlockTypeServerSchemaToolResult, ServerSchemaToolResult: b}
...
func concatChunksOfSameContentBlock(blocks []*ContentBlock) (*ContentBlock, error) {
	...
	case ContentBlockTypeServerSchemaToolCall:
		return concatContentBlockHelper(blocks, blockType,
			func(b *ContentBlock) *ServerSchemaToolCall { return b.ServerSchemaToolCall },
			concatServerSchemaToolCalls)

	case ContentBlockTypeServerSchemaToolResult:
		return concatContentBlockHelper(blocks, blockType,
			func(b *ContentBlock) *ServerSchemaToolResult { return b.ServerSchemaToolResult },
			concatServerSchemaToolResults)
...
func concatServerSchemaToolCalls(calls []*ServerSchemaToolCall) (ret *ServerSchemaToolCall, err error) {
	if len(calls) == 0 {
		return nil, fmt.Errorf("no server schema tool call found")
	}
	if len(calls) == 1 {
		return calls[0], nil
	}

	ret = &ServerSchemaToolCall{}
	...
	for _, c := range calls {
		...
		if ret.CallID == "" {
			...
			return nil, fmt.Errorf("expected call ID '%s' for server schema tool call, but got '%s'", ret.CallID, c.CallID)
		}

		if ret.Name == "" {
			ret.Name = c.Name
		} else if c.Name != "" && c.Name != ret.Name {
			return nil, fmt.Errorf("expected tool name '%s' for server schema tool call, but got '%s'", ret.Name, c.Name)
		}
		...
		if c.Arguments != nil {
			...
			if argsType == nil {
				...
				return nil, fmt.Errorf("expected type '%s' for server schema tool call arguments, but got '%s'", argsType, argsType_)
			}
...
func concatServerSchemaToolResults(results []*ServerSchemaToolResult) (ret *ServerSchemaToolResult, err error) {
	if len(results) == 0 {
		return nil, fmt.Errorf("no server schema tool result found")
	}
	if len(results) == 1 {
		return results[0], nil
	}

	ret = &ServerSchemaToolResult{}
	...
	for _, r := range results {
		...
		if ret.CallID == "" {
			...
			return nil, fmt.Errorf("expected call ID '%s' for server schema tool result, but got '%s'", ret.CallID, r.CallID)
		}

		if ret.Name == "" {
			ret.Name = r.Name
		} else if r.Name != "" && r.Name != ret.Name {
			return nil, fmt.Errorf("expected tool name '%s' for server schema tool result, but got '%s'", ret.Name, r.Name)
		}
		...
		if r.Result != nil {
			...
			if resType == nil {
				...
				return nil, fmt.Errorf("expected type '%s' for server schema tool result, but got '%s'", resType, resType_)
			}
	...
	if resChunks.IsValid() && !resChunks.IsZero() {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to concat server schema tool result: %v", err)
		}
...
func (b *ContentBlock) String() string {
	...
	case ContentBlockTypeServerSchemaToolCall:
		if b.ServerSchemaToolCall != nil {
			sb.WriteString(b.ServerSchemaToolCall.String())
		}
	case ContentBlockTypeServerSchemaToolResult:
		if b.ServerSchemaToolResult != nil {
			sb.WriteString(b.ServerSchemaToolResult.String())
		}
	case ContentBlockTypeAssistantGenText:
...
func (s *ServerSchemaToolCall) String() string {
	sb := &strings.Builder{}
	sb.WriteString(fmt.Sprintf("      name: %s\n", s.Name))
	if s.CallID != "" {
		sb.WriteString(fmt.Sprintf("      call_id: %s\n", s.CallID))
...
func (s *ServerSchemaToolResult) String() string {
	sb := &strings.Builder{}
	sb.WriteString(fmt.Sprintf("      name: %s\n", s.Name))
	if s.CallID != "" {
		sb.WriteString(fmt.Sprintf("      call_id: %s\n", s.CallID))
